...
import { Input, InputGroup, Flex, Text, Box, Center, Select, Button, extendTheme, Icon, } from "@chakra-ui/react";
...
function DormOverviewCard ({ dormId }) {
        ...
        return (
          <div style={{ display: 'flex', alignItems: 'center', margin:'auto' }}>
            {Array(fullStars)
              .fill(0)
              .map((_, index) => (
                <FaStar key={`full-${index}`} color="#A8996E" style={{ marginRight: '6px' }} />
              ))}
            {halfStars > 0 && <FaStar key="half" color="#A8996E" style={{ marginRight: '6px' }} />}
            {Array(emptyStars)
              .fill(0)
              .map((_, index) => (
                <FaStar key={`empty-${index}`} color="#e4e5e9"  style={{ marginRight: '6px' }} />
              ))}
          </div>
        );
      };
    ...
    useEffect(() => {
        if (dormId){
            ...
            const unsubscribe = onSnapshot(dormRef, (doc) => {
                if (doc.exists()) {
                    setAverages({
    ...
    return (
        <>
          {isDataLoaded && averages.overallAverageRating > 0 && (
            <Center>
              <Box borderWidth="1px" borderRadius="lg" overflow="hidden" maxW="lg" w="300px" m={7} p={5}>
                <Text as="div" fontWeight={700} fontSize="1.5rem" textAlign="center"> {/* Center align text */}
                  Overall Rating
                  <br />
                  <span style={{ display: 'flex', alignItems: 'center', fontSize: "2.5rem", justifyContent: 'center' }}> {/* Center align flex content */}
                    {averages.overallAverageRating}
                    <FaStar key="half" color="#A8996E" style={{ marginLeft: '20px', width: '40px', height: '40px' }} />
                  </span>
                </Text>
                <Text as="div" fontWeight={700} fontSize="1.3rem" mt={3} textAlign="center"> {/* Center align text */}
                  Breakdown
                </Text>
                <Box as="div" textAlign="center"> {/* Center align text */}
                <Text mb={2} mt={2}>
                    Room 
                </Text>
                <Center> {/* Center align StarRating */}
